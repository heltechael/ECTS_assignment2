...
int main(void) {
	...
	while (1) {
		...
		xil_printf("Enter choice: 1 (SW LEDS), 2 (Timer LEDS), 3 (Software MatMul), 4 (Hardware MatMul).\n\rCMD:> ");
...
//				;
		...
		case '3':
		    ...
		    xil_printf("SOFTWARE TIME: %u clock ticks\r\n", 2*(start-end)); // start minus end because counter counts down.
			break;
		case '4':
			clearMatrix(A);
			clearMatrix(B);
		    ...
		    xil_printf("HARDWARE TIME: %u clock ticks\r\n", 2*(start-end));
			break;
		default:
			break;
		}
